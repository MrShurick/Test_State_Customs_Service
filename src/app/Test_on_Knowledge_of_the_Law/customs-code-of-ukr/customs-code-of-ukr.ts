...
@Component({
  selector: 'app-customs-code-of-ukr',
  imports: [],
  templateUrl: './customs-code-of-ukr.html',
  styleUrl: './customs-code-of-ukr.scss',
})
export class CustomsCodeOfUkr {
    ...
    this.service.getTest().pipe(
      ...
      for (const catagoryKey in datas) {
        ...
        if (typeof subCatagory === 'object' && subCatagory !== null && !Array.isArray(subCatagory)) {
          ...
          const constitKey = Object.keys(typedSubCatagor).find(key => key.includes('Митного кодексу') || key.includes('Митний кодекс'));
